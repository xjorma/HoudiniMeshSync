...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    {
        //CUSTOM:
        [Min(0.0001f), Tooltip("Lets LookingGlass components drive the Focus and Apeture values.")]
        public BoolParameter snapToFocalPlane = new BoolParameter { overrideState = true, value = true };

        [Min(0.0001f), Tooltip("The intensity of the DOF effect as driven by the Holoplay Capture.")]
        public FloatParameter intensity = new FloatParameter { overrideState = true, value = 12.5f };
    ...
    internal sealed class DepthOfFieldRenderer : PostProcessEffectRenderer<DepthOfField>
    {
        ...
        {
            //CUSTOM:
            float focusDistance = settings.focusDistance.value;
            float focalLength = settings.focalLength.value;
            float aperture = settings.aperture.value;
            if (settings.snapToFocalPlane.value && context.camera.TryGetComponent(out LookingGlass.HologramCamera hologramCamera)) {
                focusDistance = hologramCamera.CameraProperties.FocalPlane;
                focalLength = Mathf.Abs(focusDistance * settings.intensity.value);
                aperture = Mathf.Abs(focusDistance / settings.intensity.value);
            }
