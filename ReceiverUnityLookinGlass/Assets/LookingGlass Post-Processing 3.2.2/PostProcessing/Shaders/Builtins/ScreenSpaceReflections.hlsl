...
Texture2D _FAKEDepthTexture; SamplerState sampler_FAKEDepthTexture;
...
{
    float depth = _FAKEDepthTexture.SampleLevel(sampler_FAKEDepthTexture, UnityStereoTransformScreenSpaceTex(uv), 0).r;
...
Result March(Ray ray, VaryingsDefault input)
{
    ...
    for (int i = 0; i < _MaximumIterationCount; ++i)
    {
        ...
        float d = _FAKEDepthTexture.SampleLevel(sampler_FAKEDepthTexture, UnityStereoTransformScreenSpaceTex(uv), 0);
...
{
    float z = _FAKEDepthTexture.SampleLevel(sampler_FAKEDepthTexture, i.texcoordStereo, 0).r;
